...
def relevant_conflicts(q):
    ...
    question touches them they are surfaced alongside current evidence and
    ...
    for x in CLAIMS:
        ...
        if state.value != 'conflicting' and meta.get('conflict_status') != 'held_back':
            continue
        claim_text = str(x.get('Claim', ''))
        if not qt or qt & tokens(claim_text + ' ' + str(x.get('Category', ''))):
            item = {
            ...
            key = str(item.get('Conflict ID')) + '|' + claim_text
            if key not in seen:
                out.append(item)
                seen.add(key)
    return out
...
def answer(q,player_context=None):
    ...
    if not gate['passes']:
        ...
        s['quality_gate']=gate
    return {'question':q,'answer_type':'evidence_fallback' if s.get('model')=='evidence-fallback' else 'synthesized_evidence','player_context':player_context,'season_scope':scope,'answer':s['text'],'model':s['model'],'evidence_used':s.get('evidence_used',[]),'uncertainty':s.get('uncertainty',''),'synthesis_error':s.get('synthesis_error'),'synthesis_error_detail':s.get('synthesis_error_detail'),'evidence':hits,'critical_conflicts':critical,'quality_gate':s.get('quality_gate',{}),'rules_applied':RULES[:4],'note':'Answer is synthesized from retrieved evidence and passed an evidence-relevance gate. Tier 1 is preferred for mechanics; conflicts and uncertainty are preserved.'}
...
class H(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if u.path=='/api/health':
            ...
            return self._json({'ok':True,'version':RELEASE,'claims':len(CLAIMS),'sources':DATA['stats'].get('sources',0),'changes':DATA['stats'].get('change_log_entries',0),'conflicts':len(CONFLICTS),'tier1':DATA['stats']['tier1_claims'],'synthesis_configured':configured,'synthesis_status':SYNTHESIS_RUNTIME_STATUS,'model':LLM_MODEL,'synthesis':synthesis,'response_engine':{'status':'ready','primary_model':'deterministic-evidence-synthesis','llm_enhancement_enabled':os.getenv('FGF_ENABLE_LLM_ENHANCEMENT','false').lower() in ('1','true','yes','on'),'external_api_required':False,'deterministic_synthesis_version':'1.5'},'adaptive_retrieval':True,'bounded_learning':True,'event_calendar_aware':True,'shared_moonlight_current':True,'claim_lifecycle_aware':True,'lifecycle_states':summarize_states(CLAIMS),'knowledge_generated':DATA.get('generated')})
        if u.path=='/api/ask':
